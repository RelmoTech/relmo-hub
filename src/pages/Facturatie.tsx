...
export function Facturatie() {
  ...
  const calcTotals = (lines: InvoiceLine[]) => {
    ...
    for (const l of lines) { const c = calcLine(l); subtotal += c.subtotal; vat += c.vat }
    return { subtotal, vat, total: subtotal + vat }
  }

  const handleSave = async () => {
    if (!modal?.client_name) return
    await upsert({ ...modal, invoice_number: modal.invoice_number || nextNumber, lines: modal.lines || [] })
    setModal(null)
  }
  ...
  const statusLabel = (s: string) => {
    if (s === 'concept') return t('concept')
    if (s === 'verzonden') return t('sent')
    if (s === 'betaald') return t('paid')
    if (s === 'vervallen') return t('overdue')
    return s
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">{t('facturatie')}</h1>
        <button
      ...
      <div className="flex gap-2">
        {(['', 'concept', 'verzonden', 'betaald', 'vervallen'] as const).map(s => (
          <button
            key={s}
            onClick={() => setStatusFilter(s)}
            className={`px-3 py-1.5 text-xs rounded-full ${statusFilter === s ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}
          >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-slate-500 border-b dark:border-slate-700">
              <th className="p-3">{t('invoiceNr')}</th>
            ...
            {filtered.map(inv => {
              ...
              return (
                <tr key={inv.id} className="border-b dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="p-3">
                    <span className={`text-xs px-2 py-1 rounded-full ${STATUS_COLORS[inv.status]}`}>{statusLabel(inv.status)}</span>
                  </td>
                  <td className="p-3 flex gap-1">
                    <button onClick={() => setViewId(inv.id)} className="p-1 text-slate-400 hover:text-blue-500"><Eye size={14} /></button>
                    <button onClick={() => setModal(inv)} className="p-1 text-slate-400 hover:text-blue-500"><Send size={14} /></button>
                    {inv.status === 'concept' && (
                      <button onClick={() => upsert({ id: inv.id, status: 'verzonden' })} className="text-xs text-blue-500">{t('markSent')}</button>
                    )}
                    {inv.status === 'verzonden' && (
                      <button onClick={() => upsert({ id: inv.id, status: 'betaald' })} className="text-xs text-emerald-500">{t('markPaid')}</button>
                    )}
                  </td>
                </tr>
              )
        ...
        {filtered.length === 0 && <p className="text-center text-slate-400 py-8 text-sm">{t('noInvoices')}</p>}
      </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={() => setModal(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 w-full max-w-2xl space-y-4 my-8" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 className="font-semibold">{t('invoice')} {modal.invoice_number}</h2>
              <button onClick={() => setModal(null)}><X size={18} /></button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <input placeholder={t('clientName')} value={modal.client_name || ''} onChange={e => setModal({ ...modal, client_name: e.target.value })} className="px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-transparent text-sm" />
              <input placeholder={t('clientEmail')} value={modal.client_email || ''} onChange={e => setModal({ ...modal, client_email: e.target.value })} className="px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-transparent text-sm" />
              <textarea placeholder={t('clientAddress')} value={modal.client_address || ''} onChange={e => setModal({ ...modal, client_address: e.target.value })} className="px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-transparent text-sm" />
              ...
              <input type="date" placeholder={t('dueDate')} value={modal.due_date || ''} onChange={e => setModal({ ...modal, due_date: e.target.value || null })} className="px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-transparent text-sm" />
            </div>

            <div>
              <h3 className="text-sm font-medium mb-2">{t('lines')}</h3>
              <div className="space-y-2">
                {(modal.lines || []).map((line, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <input placeholder={t('description')} value={line.description} onChange={e => updateLine(i, 'description', e.target.value)} className="flex-1 px-2 py-1.5 border border-slate-200 dark:border-slate-700 rounded text-xs bg-transparent" />
                    <input type="number" placeholder={t('quantity')} value={line.quantity} onChange={e => updateLine(i, 'quantity', Number(e.target.value))} className="w-16 px-2 py-1.5 border border-slate-200 dark:border-slate-700 rounded text-xs bg-transparent" />
                    <input type="number" placeholder={t('unitPrice')} value={line.unit_price} onChange={e => updateLine(i, 'unit_price', Number(e.target.value))} className="w-24 px-2 py-1.5 border border-slate-200 dark:border-slate-700 rounded text-xs bg-transparent" />
              ...
              <button onClick={addLine} className="mt-2 text-xs text-blue-500">{t('addLineBtn')}</button>
            </div>

            {modal.lines && modal.lines.length > 0 && (
              <div className="text-right text-sm space-y-1 border-t dark:border-slate-700 pt-3">
                <div>{t('subtotal')}: {formatCurrency(calcTotals(modal.lines).subtotal)}</div>
                <div>{t('vatLabel')}: {formatCurrency(calcTotals(modal.lines).vat)}</div>
                <div className="font-bold">{t('totalAmount')}: {formatCurrency(calcTotals(modal.lines).total)}</div>
              </div>
            )}

            <textarea placeholder={t('notes')} value={modal.notes || ''} onChange={e => setModal({ ...modal, notes: e.target.value })} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-transparent text-sm h-16" />
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={() => setViewId(null)}>
          <div className="bg-white rounded-xl p-8 w-full max-w-2xl my-8 text-slate-900" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between mb-8 no-print">
              <button onClick={() => setViewId(null)} className="text-sm text-slate-500">{t('close')}</button>
            ...
            <div className="flex justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-slate-500">{t('vatNumber')}: {COMPANY.vat}</p>
              </div>
              <div className="text-right">
                <h3 className="text-lg font-bold">{t('invoiceTitle')}</h3>
                <p className="text-sm">{viewInvoice.invoice_number}</p>
                <p className="text-sm text-slate-500">{t('colDate')}: {formatDate(viewInvoice.issue_date)}</p>
                {viewInvoice.due_date && <p className="text-sm text-slate-500">{t('dueDate')}: {formatDate(viewInvoice.due_date)}</p>}
              </div>
            </div>

            <div className="mb-8">
              <h4 className="text-sm font-semibold text-slate-500 mb-1">{t('client')}</h4>
            ...
            <table className="w-full text-sm mb-6">
              <thead>
                <tr className="border-b-2 border-slate-200">
                  <th className="text-left pb-2">{t('description')}</th>
                  <th className="text-right pb-2">{t('quantity')}</th>
                  <th className="text-right pb-2">{t('unitPrice')}</th>
                  <th className="text-right pb-2">{t('vatLabel')}</th>
                  <th className="text-right pb-2">{t('totalAmount')}</th>
                </tr>
              </thead>
              <tbody>
            ...
            <div className="text-right space-y-1">
              <div className="text-sm">{t('subtotal')}: {formatCurrency(calcTotals(viewInvoice.lines || []).subtotal)}</div>
              <div className="text-sm">{t('vatLabel')}: {formatCurrency(calcTotals(viewInvoice.lines || []).vat)}</div>
              <div className="text-lg font-bold border-t-2 border-slate-200 pt-2">{t('totalAmount')}: {formatCurrency(calcTotals(viewInvoice.lines || []).total)}</div>
            </div>
